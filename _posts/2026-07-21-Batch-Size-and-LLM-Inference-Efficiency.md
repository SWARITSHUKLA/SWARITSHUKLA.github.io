...
- $t_{weight-fetch}$ dosen't grow at all it remains constant
...
---
 ...
 - plotting the conjunction of both $t_{weight-fetch }$  & $t_{kv-fetch }$ as $t_{memory}$
...
- Here is a caveat, As you  increase the batch size, at some point $t_{kv-fetch }$ will surpass $t_{compute}$ and at that point compute is no longer the lower bound its memory bandwidth and the MFU (model flop utilization) decreases significantly, So the cost/token increases if the batch size is too big 

Thanks for the read :)

I wrote this article because its the most pedagogically effective way for me to understand this topic explained by Reiner Pope on [Dwarkesh patel](https://www.youtube.com/watch?v=xmkSf5IS-zw&t=160)